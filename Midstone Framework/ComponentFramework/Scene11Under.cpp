...
Scene11Under::Scene11Under() : character(nullptr), characterMesh(nullptr), shaderPtr(nullptr), texturePtr(nullptr), boxMesh(nullptr), doorLeft(nullptr), doorBottom(nullptr), doorRight(nullptr), doorTexture(nullptr), floor(nullptr), floorTexture(nullptr),
...
Scene11Under::~Scene11Under() {}

bool Scene11Under::OnCreate() {
	//Room setup
...
void Scene11Under::HandleEvents(const SDL_Event& sdlEvent) {
	character->HandleEvents(sdlEvent);
}

int Scene11Under::SetScene() {
	return sceneNumber;
}

void Scene11Under::BuildCharacter() {
...
void Scene11Under::BuildAllEnemies() {
	...
	archerEnemy->setPos(Vec3(1.5f, 2.0f, -15.0f));
	...
	demon->setPos(Vec3(-3.0f, 2.0f, -15.0f));
}

void Scene11Under::BuildRoom() {
...
void Scene11Under::BuildHealthUI() {
...
void Scene11Under::Update(const float deltaTime) {
	...
	if (archerEnemy->isAlive() == false && ratEnemy->isAlive() == false && demon->isAlive() == false || roomCleared == true) { //enemies are dead - unlock room
		roomCleared = true;
...
void Scene11Under::Render() const {
	/// Clear the screen
...
bool Scene11Under::setSceneCleared() {
	return roomCleared;
}

void Scene11Under::getSceneCleared(const bool storedRoom_) {
	roomCleared = storedRoom_;
}

float Scene11Under::setCharacterHealth() {
	return health;
}

void Scene11Under::getCharacterHealth(const float storedHealth_) {
	health = storedHealth_;
}

Vec3 Scene11Under::setCharacterPos() {
	return character->getPos();
}

void Scene11Under::getCharacterPos(const Vec3 storedPos_) {
...
float Scene11Under::setCharacterSpeed() {
	return character->getSpeed();
}

void Scene11Under::getCharacterSpeed(const float storedSpeed_) {
	speed = storedSpeed_;
}

int Scene11Under::setWeapon() {
	return weapon;
}

void Scene11Under::getWeapon(const int storedWeapon_) {
	weapon = storedWeapon_;
}

void Scene11Under::OnDestroy() {

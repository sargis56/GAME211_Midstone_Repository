...
void SceneDev::HandleEvents(const SDL_Event& sdlEvent) {
	character->HandleEvents(sdlEvent);
}

int SceneDev::SetScene() {
	return sceneNumber;
}
...
bool SceneDev::setSceneCleared() {
	return roomCleared;
}

void SceneDev::getSceneCleared(const bool storedRoom_) {
	roomCleared = storedRoom_;
}

float SceneDev::setCharacterHealth() {
	return health;
}

void SceneDev::getCharacterHealth(const float storedHealth_) {
	health = storedHealth_;
}

Vec3 SceneDev::setCharacterPos() {
	return character->getPos();
}

void SceneDev::getCharacterPos(const Vec3 storedPos_) {
...
void SceneDev::getCharacterSpeed(const float storedSpeed_) {
	speed = storedSpeed_;
}

int SceneDev::setWeapon() {
	return weapon;
}

void SceneDev::getWeapon(const int storedWeapon_) {

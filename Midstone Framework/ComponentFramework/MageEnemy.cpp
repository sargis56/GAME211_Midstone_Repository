...
MageEnemy::MageEnemy(Mesh* mesh_, Shader* shader_, Texture* texture_, Room room_, Character* character_) :
	mesh(mesh_), shader(shader_), texture(texture_), room(room_), character(character_) {
}

MageEnemy::~MageEnemy() {}

void MageEnemy::BuildProjectile() {
...
bool MageEnemy::DamageCheck(Character* character) { //function to detect dmg

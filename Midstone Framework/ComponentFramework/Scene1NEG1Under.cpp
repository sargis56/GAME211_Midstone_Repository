...
Scene1NEG1Under::Scene1NEG1Under() : character(nullptr), characterMesh(nullptr), shaderPtr(nullptr), texturePtr(nullptr), boxMesh(nullptr), doorRight(nullptr), doorTexture(nullptr), floor(nullptr), floorTexture(nullptr),
...
Scene1NEG1Under::~Scene1NEG1Under() {}

bool Scene1NEG1Under::OnCreate() {
	//Room setup
...
void Scene1NEG1Under::HandleEvents(const SDL_Event& sdlEvent) {
	character->HandleEvents(sdlEvent);
}

int Scene1NEG1Under::SetScene() {
	return sceneNumber;
}

void Scene1NEG1Under::BuildCharacter() {
...
void Scene1NEG1Under::BuildAllEnemies() {
...
void Scene1NEG1Under::BuildHealthUI() {
...
void Scene1NEG1Under::Update(const float deltaTime) {
	//enemy and item updates
	if (roomUpdate == false) {
		if (archerEnemy->isAlive()) {
			archerEnemy->Update(deltaTime);
			if (archerEnemy->getTimer() >= 20) {
				archerTexture->LoadImage("textures/Enemies/Archer_D.jpg");
...
void Scene1NEG1Under::Render() const {
	/// Clear the screen
...
bool Scene1NEG1Under::setSceneCleared() {
	return roomCleared;
}

void Scene1NEG1Under::getSceneCleared(const bool storedRoom_) {
	roomCleared = storedRoom_;
}

float Scene1NEG1Under::setCharacterHealth() {
	return health;
}

void Scene1NEG1Under::getCharacterHealth(const float storedHealth_) {
	health = storedHealth_;
}

Vec3 Scene1NEG1Under::setCharacterPos() {
	return character->getPos();
}

void Scene1NEG1Under::getCharacterPos(const Vec3 storedPos_) {
...
float Scene1NEG1Under::setCharacterSpeed() {
	return character->getSpeed();
}

void Scene1NEG1Under::getCharacterSpeed(const float storedSpeed_) {
	speed = storedSpeed_;
}

int Scene1NEG1Under::setWeapon() {
	return weapon;
}

void Scene1NEG1Under::getWeapon(const int storedWeapon_) {
	weapon = storedWeapon_;
}

void Scene1NEG1Under::OnDestroy() {

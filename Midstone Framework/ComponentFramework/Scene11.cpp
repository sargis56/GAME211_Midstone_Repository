...
Scene11::Scene11() : character(nullptr), characterMesh(nullptr), shaderPtr(nullptr), texturePtr(nullptr), boxMesh(nullptr), doorLeft(nullptr), doorBottom(nullptr), doorRight(nullptr), doorTexture(nullptr), floor(nullptr), floorTexture(nullptr),
...
void Scene11::HandleEvents(const SDL_Event& sdlEvent) {
	character->HandleEvents(sdlEvent);
}

int Scene11::SetScene() {
	return sceneNumber;
}

void Scene11::BuildCharacter() {
...
void Scene11::BuildAllEnemies() {
	...
	mage->OnCreate();
	mage->setPos(Vec3(0.0f, 0.0f, -15.0f));
	...
	demon1->setPos(Vec3(1.0f, -3.0f, -15.0f));
	...
	demon3->setPos(Vec3(3.0f, -3.0f, -15.0f));
}

void Scene11::BuildRoom() {
...
void Scene11::Update(const float deltaTime) {
...
bool Scene11::setSceneCleared() {
	return roomCleared;
}

void Scene11::getSceneCleared(const bool storedRoom_) {
	roomCleared = storedRoom_;
}

float Scene11::setCharacterHealth() {
	return health;
}

void Scene11::getCharacterHealth(const float storedHealth_) {
	health = storedHealth_;
}

Vec3 Scene11::setCharacterPos() {
	return character->getPos();
}

void Scene11::getCharacterPos(const Vec3 storedPos_) {
...
float Scene11::setCharacterSpeed() {
	return character->getSpeed();
}

void Scene11::getCharacterSpeed(const float storedSpeed_) {
	speed = storedSpeed_;
}

int Scene11::setWeapon() {
	return weapon;
}

void Scene11::getWeapon(const int storedWeapon_) {
	weapon = storedWeapon_;
}

void Scene11::OnDestroy() {
